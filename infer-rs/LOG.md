...
- **Re-baseline defaults (next):** use `scripts/bench_openssl_partial.sh` for repeated 74-file OpenSSL runs. The one-shot no-explicit-cap checkpoint is `294.94s` / `12.6GB` max RSS / `19` aborts / `max_visit_count=4`, but host noise is still large.
...
- The old "continue whole-program OpenSSL work on the publishable
  ...
  → out-of-box `~6.9× / clean` vs OCaml on the 74-file partial
