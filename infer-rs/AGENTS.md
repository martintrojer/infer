...
Active work, decisions, and resumable context live in `mu`, not in ad-hoc TODO
or log files.

- Before starting work, run `mu state -w infer-rs`.
- Represent actionable follow-ups as `mu` tasks with durable notes.
- When deleting or streamlining docs, migrate any actionable task-like bullets
  into `mu` before removing them.
- Use `docs/STATUS.md` for the current dashboard and `docs/plans/` for
  historical investigations.

Prefer `mu task note` for anything that should survive compaction or handoff.
