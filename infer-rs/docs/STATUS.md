...
bounded-visit DES-family large-state procedures. Memory-sensitive runs can
enable `--pulse-intermediate-formula-gc` to prune unreachable interval/is-int
facts in large intermediate states, trading wall time for lower RSS. Use
...
- `NEXT_STEPS.md`: A / B / C / D / E candidate work list and what's
  done so far, including B-pass fixes for widen semantics, attribute
  timestamp canonicalization, dynamic-type canonicalization,
  stopped-state identity transfer, and post-stable WTO convergence.
...
- Real-fixture latent invalid-access summary parity tightened again too:
  `FN_nonlatent_use_after_free_bad{,2}` no longer publish the stray
  `LatentInvalidAccess` summary on the exported `latent.c` fixture and now
  stay at `ContinueProgram + LatentAbortProgram` like OCaml. The final
  remaining `latent_use_after_free` shape gap is now fixed too on the
  validated exported fixture: summary export coalesces the surviving shared
  zero direct-formal latent-invalid path only after the mixed-depth /
  path-local filters accept it, and summary import now conjoins equalities
  between caller actuals when one callee dereferenced-formal summary value is
  shared by multiple formals instead of arbitrarily binding that value to the
  first actual. That restores the OCaml-looking `{b == 0}` latent-invalid
  summary shape without regressing `main` or `manifest_use_after_free` back to
  a manifest `NULLPTR_DEREFERENCE`.
...
- The remaining active work is concentrated in exact issue-set / reporting parity for broader
  ...
  `traverse_and_crash_if_equal_to_root` repro and simplified one-node caller/export path, plus
  richer trace reconstruction.
...
4. **Pulse interprocedural gaps** vs OCaml:
   ...
   - Latent invalid access now exists for caller-derived invalid addresses, imported pure-call
     ...
     `traverse_and_crash_if_equal_to_root` repro and simplified one-node caller/export path, plus
     richer suppression / trace presentation detail.
...
- latent issue type reporting / traces
  ...
  `procedure_start_line`, and empty `extras`, access-side bug traces now
  reorder caller provenance before a
